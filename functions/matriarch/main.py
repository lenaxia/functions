...
def _import_batch(
    ...
    Behaviour notes:
    - Files that don't exist on disk (e.g. download failed) are skipped.
    - Polling deadline is per-batch, not cumulative across batches.
...
def _run(
    ...
    incremental progress on large backlogs rather than trying to do all work
    before returning. `max_downloads_per_run` (optional) caps fresh downloads
    per invocation; recovered files always get imported regardless of the cap.
    """
    try:
        ...
        # Build the batch queue. Recovered files go first; then alternate
        # download + import in batches of `batch_size`.
        downloaded_total = 0
        failed_downloads = 0
        imported_total = 0
        ...
        # Flush recovered files immediately if we have a full batch or if
